"""Module containing class to manage Binance API data
...
class BinanceExchange(BaseExchange):
    ...
    candle_sticks_url = "https://api.binance.com/api/v3/klines"
    api_name = "Binance"
    _keys = [
        "TimeStamp",
        "OpenPrice",
    ...
    async def request_data(self, asset_name, start_date, end_date, session):
        ...
        try:
            query_url = f"{self.candle_sticks_url}?symbol={asset_name}USDT&interval=1m&startTime={start_time}000&endTime={end_time}000"
        ...
        try:
            ...
            print(
                f"""You have tried to access and out of range list index in {self.api_name} raw_data, please ensure sure you have requested data to fill raw_data and check that keys
                  contains the following entries 'TimeStamp', 'OpenPrice', 'HighPrice', 'LowPrice', 'ClosePrice', 'Volume': """,
                repr(error),
            )
